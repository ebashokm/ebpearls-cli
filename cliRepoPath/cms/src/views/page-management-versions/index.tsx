...
import { PageManagementWithVersionAddPath, PageManagementWithVersionEditPath } from './constants';
...
const PageList = () => {
    ...
    const { GET_ADMIN_PROFILE, GET_ALL_LIST_PAGES_WITH_VERSION, REMOVE_PAGE_WITH_VERSION } = useGQL();
    const { loading: adminProfileLoading, data: adminProfileData } = GET_ADMIN_PROFILE();
    const { loading, data, refetch } = GET_ALL_LIST_PAGES_WITH_VERSION();
    const [handleDeletePage] = REMOVE_PAGE_WITH_VERSION();
    ...
    useEffect(() => {
        if (data?.findAllPagesWithVersiion?.data) {
            setRows(data.findAllPagesWithVersiion.data);
            setCount(data.findAllPagesWithVersiion.pagination.total);
        }
    }, [data?.findAllPagesWithVersiion?.data]);
    ...
    const handleRemovePage = async () => {
        try {
            await handleDeletePage({
                variables: { removePageWithVersionId: selectedPageId }
            });
            refetch();
            ...
            title={
                <Grid container justifyContent={{ md: 'space-between' }} alignItems={{ md: 'center' }} spacing={2}>
                    <Grid item xs={12} md={5}>
                        <Typography variant="h2">Page With Versions</Typography>
                    </Grid>
                    <Grid item xs={12} md={7}>
                        <Stack>
                            <TextField
                                ...
                                size="small"
                            />
                            <Button component={Link} to={PageManagementWithVersionAddPath} variant="outlined">
                                Add Page
                            </Button>
                        </Stack>
                    </Grid>
                                                            ...
                                                            <>
                                                                ...
                                                                <Menu {...bindMenu(popupState)}>
                                                                    <MenuItem
                                                                        ...
                                                                        onClick={() => {
                                                                            navigate(`${PageManagementWithVersionEditPath}/${row._id}`);
                                                                        }}
                                                                    >
                                                                        Edit
                                                                    </MenuItem>
