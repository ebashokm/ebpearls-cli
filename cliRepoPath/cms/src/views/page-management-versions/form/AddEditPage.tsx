...
import { PageManagementWithVersionListPath } from '../constants';
import { PageStatus, PageTypes, PageTypesWithVersion } from '../constants/variables';
...
const AddEditPage = () => {
    ...
    const { CREATE_PAGE_WITH_VERSION, UPDATE_PAGE_WITH_VERSION, GET_PAGE_WITH_VERSION } = useGQL();
    const [handleCreatePage, { data }] = CREATE_PAGE_WITH_VERSION();
    const { data: pageData, loading: pagaDataLoading } = GET_PAGE_WITH_VERSION(id!);
    const [handleUpdatePage] = UPDATE_PAGE_WITH_VERSION();

    const breadcrumbLinks = [
        { title: 'Page management with versions', to: PageManagementWithVersionListPath },
        { title: id ? `Edit ${pagaDataLoading ? '' : pageData?.getPageWithVersion?.pageType} page` : 'Add page' }
    ];

    useEffect(() => {
        if (pageData?.getPageWithVersion) {
            setInitialValues({
                ...pageData?.getPageWithVersion,
                pageType: PageTypes.find((item) => item.label === pageData?.getPageWithVersion?.pageType)?.value
            });
        }
    }, [pageData]);
            ...
            try {
                ...
                await handleUpdatePage({
                    variables: {
                        body: {
                            ...others,
                            id: pageData?.getPageWithVersion?._id!
                        }
                    }
                });
                navigate(PageManagementWithVersionListPath, { state: { refetch: true } });
                                ...
                                <Grid container spacing={2}>
                                    <Grid container item lg={6} spacing={2}>
                                        <Grid item xs={12}>
                                            ...
                                            >
                                                {PageTypesWithVersion.map((option) => (
                                        ...
                                        <Grid item xs={12}>
                                            <InputLabel>Page version *</InputLabel>
                                            <TextField
                                                id="page-version"
                                                name="version"
                                    ...
                                    <Grid container item xs={12} lg={10} spacing={2}>
                                        ...
                                        <Grid item xs={12}>
                                            <InputLabel htmlFor="content">Content description *</InputLabel>
                                            <QuillEditor
                            ...
                            <Grid item xs={12} mt={2}>
                                <Paper sx={{ p: 2 }}>
                                    <Stack className="button-wrapper-row">
                                        ...
                                        <Button
                                            onClick={() => {
                                                navigate(PageManagementWithVersionListPath);
                                            }}
                                            variant="contained"
                                            color="primary"
                                            size="large"
