...
import { IconButtonWrapper, ImageWrapper } from 'views/menu/components/formik/formik.styles';
import CloseIcon from '@mui/icons-material/Close';
import EditIcon from '@mui/icons-material/Edit';
import DeleteIcon from '@mui/icons-material/Delete';
import { defaultFile, defaultValueMenu } from '../constants/menu';
...
const AddMenu = () => {
    ...
    const { CREATE_MENU, LIST_ALL_PAGES, LIST_TAXONOMIES, IMAGE_UPLOAD } = useGQL();
    const { data: taxonomiesData } = LIST_TAXONOMIES({ searchText: '', orderBy: 'title', order: 'desc', limit: 100, skip: 0 });
    const [handleCreateMenu] = CREATE_MENU();
    const { data: pagesData } = LIST_ALL_PAGES({ status: 'ACTIVE' });
    ...
    useEffect(() => {
        if (linkType == 'page') {
            const options = pagesData?.listAllPages?.pages
    ...
    const handleFormSubmit = async (values: any, setSubmitting: (isSubmitting: boolean) => void) => {
        ...
        handleCreateMenu({ variables: { input: dataToSubmit } })
            .then((success: any) => {
                handleOpenSnackbar({ message: success.data.createMenu.message, alertType: 'success' });
    ...
    const saveMenuItem = async (values, { setErrors, setStatus, setSubmitting }) => {
        setSubmitting(true);

        if (!isEdit) {
            if (modalOpenClickedFrom.clickedFrom == 'parent') {
                setMenuItemArray((current) => [...current, { ...values, id: uuid(), index: current.length }]);
    ...
    useEffect(()=>{
        if(currentSubMenuFileName && subMenuCroppedImage[`${currentSubMenuFileName}`])
          {  try {
                ...
                blobUploadHelper(blob, handleImageUpload, currentSubMenuFileName, imageType).then((result) => console.log('result====>' , result) );
    ...
    return (
        <>
            <Formik
                enableReinitialize
                initialValues={defaultValueMenu}
                        ...
                        <MainCard title="Add Menu">
                            <Grid container spacing={2}>
                                ...
                                <Grid item xs={12} lg={6}>
                                    ...
                                    <input type="file" id="menu-logo" name="logo" onBlur={handleBlur} onChange={handleUploadLogo} hidden />
                                ...
                                <Grid item xs={12} lg={6}>
                                    <FormControl fullWidth>
                                        <InputLabel id="demo-simple-select-label">Menu Position</InputLabel>
                                        <Select
                                            labelId="demo-simple-select-label"
                                            id="demo-simple-select"
            ...
            <Modal open={open} onClose={handleClose} className="menu-modal">
                <Paper>
                    <Formik initialValues={defaultItem} validationSchema={validationSchemaMenuItem} onSubmit={saveMenuItem}>
                        {({ errors, handleBlur, handleChange, handleSubmit: handleSubmitMenuItem, touched, values, setFieldValue }) => (
                                                ...
                                                <Grid item xs={8}>
                                                    <ImageWrapper>
                                                        {imageMenuLoading && imageMenuUrl ? (
                                                            ...
                                                            <img src={subMenuCroppedImage[`${values.icon}`]||""} style={{ height: '100%' }} />
                                                        )}
                                                    </ImageWrapper>
                                                    <input
                                                        id="submenu-logo"
